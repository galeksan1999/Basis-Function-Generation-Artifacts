function [S U sv tol] = hosvd_tall(T, dim, tol)
%HOSVD High Order SVD of a tall multidimensional array
% Note: the function is taken from TPTool toolbox and modified to accept
% tall multidimensional array (orig. [S U sv tol] = hosvd(T, dim, tol))
%	[S U sv tol] = HOSVD(T)
...
for i = 1:P
	if dim(i)
		A = ndim_unfold(T, i);
		% SVD based reduction of the current dimension (i) of the tall array
		[Ui svi toli] = svdtrunc_tall(A, tol(i));
		U{i} = Ui;
		UT{i} = Ui';
		sv{i} = svi;
		tol(i) = toli;
